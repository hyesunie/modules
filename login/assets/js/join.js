...
export default class Join {
  ...
  detailAgreementContainer = document.querySelector(
    ".detail-agreement-content"
  );
  ...
  subscribeInput() {
    ...
    this.idInput.addEventListener("input", (e) => {
      const idMessage = this.createMessageElement(
        "check-id",
        this.idInput.parentNode
      );
      idMessage.innerHTML = this.checkIdFormat(e.target.value)
        ? "멋진 아이디네요!"
        : "5~20자의 영문 소문자, 숫자와 특수기호(_),(-)만 사용 가능합니다.";
    });

    this.passwordInput1.addEventListener("input", (e) => {
      const passwordMessage = this.createMessageElement(
        "check-password1",
        this.passwordInput1.parentNode
      );
      const checkMessage = {
    ...
    this.passwordInput2.addEventListener("input", (e) => {
      const passwordMessage = this.createMessageElement(
        "check-password2",
        this.passwordInput2.parentNode
      );
    ...
    // document.querySelector(".user-info__birth").addEventListener("")

    this.birthYearInput.addEventListener("input", (e) => {
      const isTrue = this.checkBirthYearFormat(e.target.value);
      let messageSpan = document.querySelector(".check-birth");

      if (isTrue && messageSpan) {
        messageSpan.remove();
        return;
      }

      messageSpan = this.createMessageElement(
        "check-birth",
        e.target.parentNode.parentNode
      );
      messageSpan.innerHTML = "태어난 년도 4자리를 정확하게 입력하세요.";
    });

    document
      .querySelector(".user-info__birth .day")
      .addEventListener("input", (e) => {
        const month = document.querySelector(".user-info__birth #month").value;
        this.dayNumber = this.dayNumberOfMoth[month];

        const isTrue = this.checkBirthDayFormat(e.target.value);
        let messageSpan = document.querySelector(".check-birth");
        ...
        if (!isTrue) {
          ...
          messageSpan.innerHTML = "태어난 날짜를 다시 확인해주세요.";
        }
      });
    ...
    this.emailInput.addEventListener("input", (e) => {
      ...
      if (isTrue) {
        ...
        const emailMessage = this.createMessageElement(
          "check-email",
          this.emailInput.parentNode
        );
    ...
    this.interestInput.addEventListener("input", (e) => {
      ...
      if (count >= 3) {
        ...
        const interestMessage = this.createMessageElement(
          "check-interest",
          this.interestInput.parentNode
        );
    ...
    this.interestInput.addEventListener("keyup", (e) => {
      if (
        ...
        e.target.parentNode.childElementCount > 1
      ) {
  ...
  checkBirthYearFormat(year) {
    const rangeLength = year.length === 4;
    const rangeYear = Number(year) <= 2007 && Number(year) >= 1923;

    if (rangeLength && rangeYear) {
      return true;
    }
  ...
  checkBirthDayFormat(day) {
    const rangeDay = Number(day) >= 1 && Number(day) <= this.dayNumber;

    console.log(typeof this.dayNumber, day, rangeDay);
    if (rangeDay) {
      return true;
    }
    return false;
